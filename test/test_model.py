...
@pytest.fixture(params=[(2, 1), (3, 2), (4, 3), (5, 4)])
...
def test_subpixel_conv_block_output_shape(subpixel_conv_block_params):
    ...
    subpixel_conv_block = SubPixelConvolutionalBlock(
        kernel_size=kernel_size, n_channels=n_channels, scaling_factor=scaling_factor
    )
...
def test_sr_resnet_output_shape(sr_resnet_params):
    ...
    sr_resnet = SuperResolutionResNet(
...
def test_generator_output_shape(sr_resnet_params):
    ...
    mock_output_tensor = MockTensor(
        shape=(
            dummy_input.shape[0],
            dummy_input.shape[1] * scaling_factor,
            dummy_input.shape[2] * scaling_factor,
            dummy_input.shape[3],
        )
    )
